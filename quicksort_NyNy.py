...
def quickSort(unsortedlist, cmpfunction):
        ...
        low1_len=listLen(low1)
        high1_len=listLen(high1)
        # Create sortedlist, first input low1 (lower than pivot)
        for j in range(low1_len): 
...
def listLen(aList):
        counter=0
        position=aList.first()
        lastposition=aList.end()
